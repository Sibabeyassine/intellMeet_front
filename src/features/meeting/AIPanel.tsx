...
import { mockActions, mockSummary } from "./mock";
      ...
      <ScrollArea className="flex-1">
        <div className="space-y-5 p-5">
          ...
          <div>
            ...
            <h4 className="mt-1 font-display text-base font-semibold text-foreground">{mockSummary.topic}</h4>
          </div>

          {/* Live summary */}
          <Section icon={FileText} title="Résumé en direct" badge="Auto">
            <ul className="space-y-2.5">
              {mockSummary.highlights.map((h, i) => (
          ...
          {/* Action items */}
          <Section icon={ListChecks} title="Action items" badge={`${mockActions.length}`}>
          ...
          <div className="rounded-2xl border border-primary/30 bg-gradient-to-br from-primary/10 via-primary/5 to-transparent p-4">
            ...
            <p className="mt-2 text-sm text-foreground/90">
              Tu veux que je planifie automatiquement la review de jeudi 15h dans Google Calendar et envoie une invitation aux 6 participants ?
            </p>
            <div className="mt-3 flex gap-2">
              <Button size="sm" className="bg-gradient-accent text-primary-foreground hover:brightness-110">
